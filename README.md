<h1>Learning Management System (LMS)</h1>
<p>The LMS isafull-featured web platform that facilitates online learning. It allows
Admins to manage users and system-wide content, Instructors to create and
manage courses, and Students to enroll in courses, consume content, take quizzes,
and track their learning progress. The system supports role-based access control,
secure login with JWT, and a dynamic, responsive frontend interface.</p>
What to do?
Authentication and Authorization
Handles login and signup using JWT. Users can be Admins, Instructors, or Students.
Each role gets access only to specific parts of the system.
Admin Module
Admins can manage all users and courses. They can view, delete, or update user
roles and approve or remove courses created by instructors.
Instructor Module
Instructors can create and manage courses. They can add lessons to each course
and see which students have enrolled.
Student Module
Students can browse available courses,enroll in them, and view lessons. They can
also track their learning progress.
Course and Lesson Management
Courses include titles and descriptions. Each course has multiple lessons, which
can contain text, videos, or PDFs.
Enrollment Module
Students enroll in courses. Once enrolled, they get access to the course materials.
Duplicate enrollments are not allowed.
